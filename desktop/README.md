...
- **系统托盘常驻**：点关闭窗口不会退出应用，而是隐藏到系统托盘，后台 `dsh web` 服务继续运行，
  浏览器随时访问 `http://127.0.0.1:5180` 仍是同一个服务的实时会话。
- **取消/停止服务**：右键托盘图标 →「停止服务并退出」，即停掉后台服务并退出应用
  （这是关停当前这个常驻服务的唯一明确入口）。
- 托盘菜单还有「打开窗口」和「浏览器打开 5180」。
...
壳与核心完全分离：`main.js` 只负责「拉起 dsh web 服务 + 开窗口/托盘」。日后换 Tauri 壳时，`dsh web` 启动逻辑整体复用，仅替换窗口/托盘载体即可。
