...
| `modelChoices` | Model-facing per-call model selection. When set (a non-empty, distinct list of model ids), the tool schema exposes an optional `model` parameter (an enum of these values) so the calling model can choose the child's executor model per call; the chosen value merges into the call's `agentOptions.model`, overriding the config-time `agentOptions.model`. Omission keeps today's behavior: no `model` parameter, executor bound at config time. The in-process provider overrides inherited parent options, so a per-call model is honored for `spawn`/`fork`; a remote provider owns its own child budget, so honoring it is that provider's responsibility. An empty or duplicate list fails at load. |
